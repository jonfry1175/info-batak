...
const REQUIRED_FIELDS = [
    'margaId', 'slug', 'sejarah', 'asalUsul', 'tarombo',
    'wilayah', 'tradisi', 'tokoh', 'relatedMargas', 'updatedAt'
];
    ...
    data.forEach((item, index) => {
        ...
        if (item.tarombo && Array.isArray(item.tarombo.ancestors)) {
            if (item.tarombo.ancestors.length < MIN_ANCESTORS) {
                errors.push(`tarombo.ancestors has ${item.tarombo.ancestors.length} items, min ${MIN_ANCESTORS}`);
            }
        }
        ...
        if (item.tarombo && Array.isArray(item.tarombo.subMargas)) {
            if (item.tarombo.subMargas.length < MIN_SUBMARGAS) {
                ...
                errors.push(`tarombo.subMargas has ${item.tarombo.subMargas.length} items, min ${MIN_SUBMARGAS}`);
            }
        }
