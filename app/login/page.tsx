...
function getOAuthErrorMessage(
  ...
  switch (errorCode) {
    ...
    default:
      // If we have a description, use it; otherwise generic message
      if (errorDescription) {
...
function LoginPageContent() {
  ...
  // Handle OAuth redirect callback - derive error from URL params using useMemo
    ...
    <div className="flex min-h-[60vh] items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Masuk ke InfoBatak.id</CardTitle>
          <CardDescription>
            Masuk untuk mengakses fitur lengkap dan menyimpan preferensi Anda
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Button onClick={handleGoogleSignIn} disabled={isSigningIn} className="w-full" size="lg">
            {isSigningIn ? (
              <>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background text-muted-foreground px-2">Belum punya akun?</span>
            </div>
          </div>
