    ...
    await fc.assert(
      fc.asyncProperty(fc.constantFrom(...allMarga), async (marga: Marga) => {
        try {
          ...
          if (detail?.sejarah) {
            expect(screen.getByRole('heading', { name: 'Sejarah' })).toBeInTheDocument();
            expect(screen.getByText(detail.sejarah)).toBeInTheDocument();
          } else {
            expect(screen.queryByRole('heading', { name: 'Sejarah' })).not.toBeInTheDocument();
          }
          ...
          if (detail?.tarombo) {
            ...
            if (detail.tarombo.ancestors?.length) {
              detail.tarombo.ancestors.forEach((ancestor) => {
                expect(screen.getAllByText(ancestor.nama).length).toBeGreaterThan(0);
              });
            }

            if (detail.tarombo.subMargas?.length) {
              detail.tarombo.subMargas.forEach((sub) => {
                expect(screen.getAllByText(sub.nama).length).toBeGreaterThan(0);
              });
            }
          } else {
          ...
          if (detail?.tokoh?.length) {
            ...
            detail.tokoh.forEach((tokoh) => {
              expect(screen.getAllByText(tokoh.nama).length).toBeGreaterThan(0);
            });
          } else {
